...
export class EmployeeHeldupInfoSetup implements OnInit {
  ...
  heldupType: any[] = [
    { key: 'SALARY', value: 'Salary Heldup' },
    { key: 'BONUS', value: 'Bonus Heldup' },
    { key: 'INCREMENT', value: 'Increment Heldup' },
    { key: 'LFA_DISBURSEMENT', value: 'LFA Disbursement Heldup' }
  ];

  selectedColumns = ['salaryHead', 'amount', 'value', 'heldupAmount'];

  customColumnNames = {
    salaryHead: 'Salary Head',
    amount: 'Amount',
    value: 'Heldup Percentage(%)',
    heldupAmount: 'Heldup Amount',
  };
  ...
  private loadHeldupDataFromList(heldupData: any) {
    ...
    this.frmGroup.patchValue({
      heldupType: heldupData.heldupType,
    ...
    if (heldupData.heldupDetails && heldupData.heldupDetails.length) {
      ...
      const gridData = heldupData.heldupDetails.map((item: any) => ({
        ...
        heldupAmount: item.heldupAmount,
        _selected: true,
      }));
  ...
  onGridDataChanged(updatedRows: any[]): void {
    ...
    const validatedRows = updatedRows.map((row) => {
      ...
      return {
        ...row,
        value,
        heldupAmount: (row.amount * value) / 100,
      };
    });
  ...
  submit() {
    ...
    // Calculate total heldup amount
    const totalHeldupAmount = this.dataSource()
      .reduce((sum, row) => sum + Number(row.heldupAmount || 0), 0);

    const payload: any = {
      employeeId: empData.id,
      heldupType: formValue.heldupType,
      // fromDate: this.formatDate(formValue.fromDate),
      // toDate: this.formatDate(formValue.toDate),
      fromDate: formValue.fromDate,
      toDate: formValue.toDate,
      remarks: formValue.remark || '',
      totalHeldupAmount: totalHeldupAmount,
    };
    ...
    if (this.dataSource().length) {
      payload.heldupDetails = this.dataSource().map((row) => ({
        ...
        heldupAmount: Number(row.heldupAmount),
      }));
    }
