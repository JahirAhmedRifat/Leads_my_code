...
//    }
    ...
    @Override
    public PageResponse<EmployeeBonusProfileResponse> getDataForBonusDisbursement(
            EmployeeBonusProfileSearchRequest request
    ) {
        ...
        // STEP 1: Get paginated profile IDs
        Page<Long> idPage = bonusProfileRepository.getProfileIdsForDisbursement(
                request.getEmpOrgId(),
                request.getEmpBranchId(),
                request.getBonusYear(),
                request.getBonusType(),
        ...
        if (idPage.isEmpty()) {
            throw new CustomException(
                    ...
                    "No draft bonus data found for disbursement"
            );
        }

        // STEP 2: Fetch profiles with only DRAFT details
        List<EmployeeBonusProfile> profiles =
                bonusProfileRepository.findProfilesWithDraftDetails(
                        idPage.getContent(),
                        "DRAFT"
                );

        // Convert to response DTO
        List<EmployeeBonusProfileResponse> responses =
                profiles.stream()
                        .map(entity -> convertToTarget(entity, EmployeeBonusProfileResponse.class))
                        .toList();

        // Build PageResponse
        Page<EmployeeBonusProfileResponse> responsePage =
                new PageImpl<>(responses, pageable, idPage.getTotalElements());
